# Output Templates

## Full Video Note

```markdown
# Video - {channel} - {short_title}

## Metadata

- Channel:
- Title:
- URL:
- Video date:
- Access date:
- Language:
- Transcript type: official / automatic / unavailable / local transcription
- Local transcript file:

## Executive Summary

## Key Ideas

## Tools, Authors, Studies, or References Mentioned

## Applicability for the User

## Links With Existing Notes Or Projects

## Useful Excerpts

Use short excerpts only. Avoid long verbatim transcript reproduction.

## Critical Evaluation

## Unverified Claims

## Limitations
```

## Channel / Recurring Monitoring Report

```markdown
# YouTube Research Report - YYYY-MM-DD HHMM

## Mode

## Priority Channels

## Complementary Channels

## Videos Located

## Videos Analyzed

## High Relevance Videos

## Medium Relevance Videos

## Skipped Videos

## Videos Without Transcript

## Notes Created

## Files Edited

## Claims Requiring Primary-Source Verification

## Limitations And Errors

## Ready-To-Send Summary

## Next Actions
```
